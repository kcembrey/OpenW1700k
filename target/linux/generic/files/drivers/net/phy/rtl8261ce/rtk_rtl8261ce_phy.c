...
struct rtl8261ce_priv {
	...
	/*
	 * Board-level SerDes polarity requests from realtek,pnswap-tx/rx.
	 * These are applied after the vendor init flow because that flow
	 * rewrites the SerDes polarity registers.
	 */
	bool pnswap_tx;
	bool pnswap_rx;
};
...
/*
 * Build the polarity request from DT properties and apply it if required.
 * This wrapper keeps optional board wiring separate from the mandatory vendor
 * init path.
 */
...
/*
 * Allocate per-PHY state and capture board lane polarity properties. The
 * properties are not programmed here because the vendor init flow rewrites the
 * same SerDes registers later in config_init().
 */
static int rtl8261ce_probe(struct phy_device *phydev)
{
	...
	/*
	 * These names are shared with other Realtek PHY drivers, but for CE the
	 * values must later be translated through the CE SerDes OCP sequence.
	 */
					   ...
					   struct rtl8261ce_priv *priv)
{
	 ...
	 */
	rtl8261ce_apply_board_serdes_polarity(phydev, priv);
...
static int rtl8261ce_config_init(struct phy_device *phydev)
{
	...
	if (priv->initialized)
		return rtl8261ce_restore_runtime_state(phydev, priv);
	...
	ret = rtl8261ce_restore_runtime_state(phydev, priv);
	if (ret < 0)
		return ret;
